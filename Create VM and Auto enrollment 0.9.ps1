...
    Write-Host " (Done)" -F Y     
    New-VHD -Path "D:\ClusterStorage\Volume1\$VMName\$VMName.vhdx” -ParentPath "D:\ClusterStorage\Volume1\Hard Disk.vhdx" -Differencing | Out-Null
...
    Write-Host @greenCheck
    Write-Host " (Done)" -F Y     
    Add-VMHardDiskDrive -VMName $VMName -ControllerType IDE -ControllerNumber 0 -Path "D:\ClusterStorage\Volume1\$VMName\$VMName.vhdx”
...
    Write-Host @greenCheck
    Write-Host " (Done)" -f Y     
    Invoke-Command -VMName $VMName -Credential $cred -ScriptBlock {Install-PackageProvider -Name NuGet -Confirm:$false -Force:$true} | Out-Null
